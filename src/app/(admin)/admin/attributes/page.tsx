        ...
        <form onSubmit={handleSave} className="space-y-4 pt-1 text-xs text-slate-800 dark:text-slate-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select
                value={formType}
                onChange={(e) => setFormType(e.target.value as "button" | "select" | "color" | "radio")}
              ...
              <div className="flex flex-wrap gap-2">
                {formValuesText
                  ...
                  .map((val, idx) => (
                    <span
                      key={idx}
                      className="text-xs font-medium px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 shadow-xs"
                    >
                      {val}
                    </span>
                  ))}
