...
import { Link2Off, RefreshCcw, LogIn } from "lucide-react";
    ...
    <div className="bg-white border border-slate-200/90 rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-[0_12px_36px_rgba(15,23,42,0.06)] space-y-6 text-center">
      ...
      <div className="space-y-1.5">
        <span className="text-[11px] font-semibold text-amber-700 uppercase tracking-wider bg-amber-50 px-3 py-1 rounded-full border border-amber-100 inline-block">
          {isSpanish ? "Enlace Expirado" : "Link Expired"}
        </span>
        ...
        <p className="text-xs text-slate-500 leading-relaxed max-w-sm mx-auto">
          {isSpanish
            ? "Este enlace ya fue utilizado o caducó por seguridad. Los enlaces temporales son válidos por 60 minutos."
            : "This verification or reset link has already been used or expired. Security links are valid for 60 minutes."}
        </p>
      </div>
      ...
      <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
        ...
        >
          ...
          <span>{isSpanish ? "Solicitar Nuevo Enlace" : "Request New Link"}</span>
        </Link>
        <Link
          href="/auth/login"
