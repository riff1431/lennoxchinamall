    ...
    <div className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 text-center">
      ...
      <div className="space-y-2">
        <span className="text-[10px] font-mono font-bold text-[#FF1028] uppercase tracking-widest bg-red-500/10 px-3 py-1 rounded-full border border-red-500/20">
          {isSpanish ? "Estado de Cuenta: Suspendida" : "Account Status: Suspended"}
        </span>
        ...
        <p className="text-xs text-slate-400 leading-relaxed max-w-sm mx-auto">
          {isSpanish
            ? "Tu cuenta de Lennox ChinaMall ha sido restringida por moderación de confianza y seguridad. Los pedidos activos y el depósito en garantía Binance Pay permanecen seguros mientras están bajo revisión."
      ...
      <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 text-left text-xs text-slate-300">
        <div className="flex items-center gap-2 font-bold text-white font-heading">
          ...
          <span>{isSpanish ? "¿Necesitas apelar o consultar pedidos activos?" : "Need to appeal or query active orders?"}</span>
        </div>
        <p className="text-[11px] text-slate-400 leading-relaxed">
          {isSpanish
            ? "Contacta a nuestro equipo global de soporte 24/7 indicando tu correo electrónico registrado para asistencia inmediata."
      ...
      <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
        ...
        >
          ...
          <span>{isSpanish ? "Contactar Seguridad y Confianza" : "Contact Trust & Safety"}</span>
        </a>
        <Link
          href="/"
        ...
        >
          {isSpanish ? "Volver a la Tienda" : "Return to Storefront"}
        </Link>
      </div>
    </div>
  );
