...
export function CategoriesDirectoryClient({ categories }: CategoriesDirectoryClientProps) {
  ...
  const breadcrumbItems = [
    {
      label: isSpanish ? "Todos los Departamentos" : "All Departments & Sourcing Hubs",
      href: "/categories",
    },
  ];
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <Breadcrumbs items={[{ label: isSpanish ? "Todos los Departamentos" : "All Departments & Sourcing Hubs" }]} />

      <div className="text-center max-w-xl mx-auto space-y-2 py-4">
        <h1 className="text-2xl sm:text-3xl font-black text-slate-900">
          {isSpanish ? "Departamentos Directos de Fábrica de China" : "Direct China Factory Departments"}
        </h1>
        <p className="text-xs text-slate-500">
          {isSpanish
            ? "Explora categorías de manufactura verificadas conectadas directamente al Portal de Compras Lennox."
        ...
        {categories.map((cat) => (
          ...
          >
            <div className="relative h-48 bg-slate-100 overflow-hidden">
              ...
              <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between">
                ...
                <span className="text-xs text-orange-400 font-bold bg-black/40 backdrop-blur-xs px-2 py-0.5 rounded-md border border-white/10 font-mono">
                  {cat.product_count}+ {isSpanish ? "Artículos" : "Items"}
                </span>
              </div>
            </div>
            ...
            <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
              ...
              >
                <span>{isSpanish ? "Explorar Departamento" : "Explore Department"}</span>
