...
export function AnalyticsDashboardClient({
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-12 font-sans text-slate-900 dark:text-slate-100">
      {/* ── 1. Page Header ── */}
      <AdminPageHeader
        ...
        breadcrumbs={[{ label: "Admin", href: "/admin/dashboard" }, { label: "Analytics & Telemetry" }]}
        actions={[
          {
            label: "Export CSV",
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pt-4 border-t border-slate-100 dark:border-slate-800/80 flex-wrap">
          {/* Preset Time Range Buttons */}
          <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-1.5 rounded-2xl overflow-x-auto max-w-full">
            {(
              [
          ...
          {/* Custom Date Pickers */}
          {timeRange === "custom" && (
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-1.5 rounded-2xl animate-in fade-in">
              <input
                type="date"
                value={customStart}
                onChange={(e) => setCustomStart(e.target.value)}
                className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 text-xs px-2.5 py-1.5 rounded-xl outline-none focus:border-[#2F65F6]"
              />
              ...
              <button
                ...
                className="bg-[#2F65F6] text-white text-xs font-bold px-3 py-1.5 rounded-xl hover:bg-[#2563EB] transition-colors cursor-pointer"
              >
                Apply
              </button>
            </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={cn(
                ...
                compareWithPrevious
                  ? "bg-[#EEF4FF] dark:bg-blue-950/60 border-blue-200 dark:border-blue-900/40 text-[#2F65F6]"
            ...
            {/* Live Indicator */}
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 px-3 py-1.5 rounded-xl text-xs">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
              </span>
              <span className="text-[11px] font-bold text-slate-600 dark:text-slate-300">
                {isRealtimeActive ? "Realtime Live" : "Connected"}
              </span>
              <button
                onClick={() => fetchData()}
                disabled={isRefreshing}
                title="Refresh Analytics"
                className="text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer ml-1"
              >
                <RefreshCw className={cn("w-3.5 h-3.5", isRefreshing && "animate-spin text-[#2F65F6]")} />
              </button>
            </div>
          </div>
        </div>
            ...
            <button
              ...
              className={cn(
                ...
                isActive
                  ...
                  : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:border-slate-300 dark:hover:border-slate-700"
              )}
            >
      ...
      {activeTab === "overview" && (
        <div className="space-y-6 animate-in fade-in duration-300">
          {/* Top 4 Primary KPI Scorecards (Pastel NETIC Cards) */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Gross Revenue */}
            <div className="p-5 rounded-2xl bg-[#F0FDF4] dark:bg-[#162720] border border-[#BBF7D0]/50 dark:border-emerald-900/30 flex items-center justify-between shadow-xs">
                ...
                {compareWithPrevious && (
                  <div className="flex items-center gap-1.5 text-xs font-bold">
                    {data.overview.grossRevenue.isPositive ? (
                      <span className="text-[#16A34A] dark:text-emerald-400 flex items-center gap-0.5">
                    ...
                    <span className="text-slate-400 font-normal">vs prior (${data.overview.grossRevenue.previous.toFixed(0)})</span>
                  </div>
                )}
                <div className="text-[11px] text-slate-400 font-medium">100% Settled via Binance Pay</div>
              </div>
              <div className="w-10 h-10 rounded-full bg-[#10B981] text-white flex items-center justify-center shadow-xs shrink-0 self-start">
                <Coins className="w-5 h-5" />
              </div>
            </div>

            {/* Total Orders */}
            <div className="p-5 rounded-2xl bg-[#EEF4FF] dark:bg-[#172033] border border-[#BFDBFE]/50 dark:border-blue-900/30 flex items-center justify-between shadow-xs">
              <div className="space-y-1">
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">
                  Sourced Orders
                </span>
                ...
                {compareWithPrevious && (
                  <div className="flex items-center gap-1.5 text-xs font-bold">
                    ...
                    <span className="text-slate-400 font-normal">{data.overview.paidOrders.current} paid dispatch</span>
                  </div>
                )}
                <div className="text-[11px] text-slate-400 font-medium">Direct factory wholesale dispatch</div>
              </div>
              <div className="w-10 h-10 rounded-full bg-[#2F65F6] text-white flex items-center justify-center shadow-xs shrink-0 self-start">
                <ShoppingCart className="w-5 h-5" />
              </div>
            </div>

            {/* Average Order Value (AOV) */}
            <div className="p-5 rounded-2xl bg-[#FFF8EE] dark:bg-[#2B2216] border border-[#FED7AA]/50 dark:border-amber-900/30 flex items-center justify-between shadow-xs">
              <div className="space-y-1">
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">
                  Avg Order Value (AOV)
                </span>
                <div className="text-2xl font-black text-slate-900 dark:text-white font-mono tracking-tight">
                  ...
                  <span className="text-xs text-slate-400 font-sans font-normal">USDT</span>
                </div>
                {compareWithPrevious && (
                  <div className="flex items-center gap-1.5 text-xs font-bold">
                    ...
                    <span className="text-slate-400 font-normal">basket size</span>
                  </div>
                )}
                <div className="text-[11px] text-slate-400 font-medium">Uplift from multi-battery drone kits</div>
              </div>
              <div className="w-10 h-10 rounded-full bg-[#F59E0B] text-white flex items-center justify-center shadow-xs shrink-0 self-start">
                <DollarSign className="w-5 h-5" />
              </div>
            </div>

            {/* Gross Sourcing Profit / Net Margin */}
            <div className="p-5 rounded-2xl bg-[#FFF0F2] dark:bg-[#2B171B] border border-[#FFE4E8]/50 dark:border-rose-900/30 flex items-center justify-between shadow-xs">
              <div className="space-y-1">
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">
                ...
                <div>
                  {data.isSuperAdmin && data.overview.grossProfit ? (
                    <>
                      ...
                      <div className="flex items-center gap-1.5 text-xs font-bold text-rose-600 dark:text-rose-400 mt-1">
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-1.5 text-xs font-bold text-[#2F65F6] mt-1">
                ...
                <div className="text-[11px] text-slate-400 font-medium">
                  {data.isSuperAdmin ? "Checkout vs Factory Cost" : "Super Admin Restricted"}
                </div>
              </div>
              <div className="w-10 h-10 rounded-full bg-[#E11D48] text-white flex items-center justify-center shadow-xs shrink-0 self-start">
          ...
          <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-5 shadow-xs">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-black text-slate-900 dark:text-white">Performance Trajectory &amp; Comparison</h3>
                </div>
              ...
              {/* Metric Switcher */}
              <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-1 rounded-xl">
                {(
                  [
            ...
            {/* SVG Line / Area Chart */}
            <div className="relative w-full overflow-hidden bg-slate-50/50 dark:bg-slate-950/60 rounded-2xl p-4 border border-slate-100 dark:border-slate-800/80">
              <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mb-2 font-mono">
                <div className="flex items-center gap-4">
                  <span className="flex items-center gap-1.5 text-slate-900 dark:text-white font-bold">
                    <span className="w-3 h-3 rounded-full bg-[#2F65F6]" />
                    Current Period ({data.dateRange.label})
                  </span>
                  {compareWithPrevious && (
                    <span className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400">
                      <span className="w-3 h-3 rounded-full bg-slate-400 border border-dashed border-slate-400" />
              ...
              >
                <defs>
                  <linearGradient id="chartGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#2F65F6" stopOpacity="0.30" />
                ...
                {chartPoints.pointsCurrent?.map((pt, idx) => (
                  <g key={idx} className="group cursor-pointer">
                    <circle
                      ...
                      cy={pt.y}
                      r="4"
                      className="fill-[#2F65F6] stroke-white dark:stroke-slate-900 stroke-2 hover:r-6 transition-all"
                    />
                  </g>
                ))}
              </svg>
              ...
              <div className="flex justify-between items-center text-[10px] font-mono text-slate-400 pt-2 border-t border-slate-100 dark:border-slate-800/60">
          ...
          <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-5 shadow-xs">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-black text-slate-900 dark:text-white">Storefront Sourcing &amp; Checkout Funnel</h3>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Full buyer progression from landing page to zero-fee Binance Pay settlement
                </p>
              </div>
              <span className="bg-[#F0FDF4] dark:bg-emerald-950/60 text-[#16A34A] dark:text-emerald-400 border border-[#BBF7D0] dark:border-emerald-900/30 text-[10px] font-bold uppercase px-3 py-1 rounded-full font-mono">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
              {data.conversionFunnel.map((stage, idx) => (
                <div
                  key={idx}
                  className="bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800/80 rounded-2xl p-4 space-y-3 relative group hover:border-slate-300 dark:hover:border-slate-700 transition-colors"
                >
                  <div className="flex items-center justify-between text-[11px] font-bold text-slate-500 dark:text-slate-400">
                    <span>Stage #{idx + 1}</span>
                    <span className="font-mono text-[#16A34A] dark:text-emerald-400">{stage.conversionPct}</span>
                  </div>
                  <div className="space-y-1">
                    <div className="text-lg font-black text-slate-900 dark:text-white font-mono">{stage.count.toLocaleString()}</div>
                    <div className="text-xs font-semibold text-slate-700 dark:text-slate-300 line-clamp-2">{stage.stage}</div>
                  </div>
                  <div className="w-full h-1.5 bg-slate-200 dark:bg-slate-900 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-[#2F65F6] to-[#10B981] rounded-full"
                      style={{ width: stage.conversionPct }}
                    />
                  </div>
                  {idx > 0 && (
                    <div className="text-[10px] text-slate-400 font-mono">
                      Dropoff: <span className="text-rose-500 font-bold">{stage.dropoffPct}</span>
                    </div>
                  )}
                </div>
              ))}
      ...
      {activeTab === "financials" && (
        <div className="space-y-6 animate-in fade-in duration-300">
          {!data.isSuperAdmin && (
            <div className="bg-[#FFF8EE] dark:bg-[#2B2216] border border-[#FED7AA] dark:border-amber-900/30 rounded-2xl p-5 flex items-center gap-4 text-amber-800 dark:text-amber-300">
              <ShieldAlert className="w-6 h-6 shrink-0 text-amber-500" />
              <div>
                <h4 className="text-sm font-bold">Confidential Sourcing Data Redacted</h4>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-5 shadow-xs lg:col-span-2">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Factory className="w-5 h-5 text-[#2F65F6]" />
                    <h3 className="text-base font-black text-slate-900 dark:text-white">Direct China Factory Sourcing P&amp;L</h3>
                  </div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Wholesale acquisition spread across Guangdong &amp; Zhejiang industrial hubs</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 space-y-1">
                  <span className="text-xs font-bold text-slate-500 dark:text-slate-400 block">Total Factory PO Cost</span>
                  <span className="text-2xl font-black text-slate-900 dark:text-slate-200 font-mono">
                  ...
                  <span className="text-[11px] text-slate-400 block">Shenzhen &amp; Guangzhou wholesale batches</span>
                </div>

                <div className="bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 space-y-1">
                  <span className="text-xs font-bold text-slate-500 dark:text-slate-400 block">Customer Retail Sourced Value</span>
                  <span className="text-2xl font-black text-[#16A34A] dark:text-emerald-400 font-mono">
                  ...
                  <span className="text-[11px] text-slate-400 block">Customer USDT checkout sum</span>
                </div>
              </div>
                  ...
                  {data.sourcingAndProfit.sourcingChannels.map((plat, idx) => (
                    <div key={idx} className="space-y-1.5">
                      ...
                      <div className="w-full h-2.5 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden border border-slate-200 dark:border-slate-800">
                        <div
                          className="h-full bg-gradient-to-r from-[#2F65F6] to-blue-400 rounded-full"
            ...
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-5 shadow-xs flex flex-col justify-between">
              <div className="space-y-4">
                <div className="border-b border-slate-100 dark:border-slate-800 pb-4 space-y-1">
                  <h3 className="text-base font-black text-slate-900 dark:text-white">Cost &amp; Fee Deductions</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Estimated overhead and gateway savings</p>
                </div>
                ...
                <div className="space-y-3 text-xs">
                  <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                    <span className="text-slate-500 dark:text-slate-400">Air Cargo Freight (DDP)</span>
                  ...
                  <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                    <span className="text-slate-500 dark:text-slate-400">Binance Pay Gas Saved (1.5%)</span>
                    <span className="font-mono font-bold text-[#16A34A] dark:text-emerald-400">
                      +${data.sourcingAndProfit.binancePayGasSavedUSDT.toFixed(2)} USDT
                    </span>
                  </div>

                  <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                    <span className="text-slate-500 dark:text-slate-400">USDT Escrow Working Balance</span>
                    <span className="font-mono font-bold text-[#2F65F6]">
              ...
              {data.isSuperAdmin && data.overview.estimatedNetProfit && (
                <div className="p-4 rounded-2xl bg-[#F0FDF4] dark:bg-[#162720] border border-[#BBF7D0] dark:border-emerald-900/30 space-y-1">
                  <span className="text-[11px] font-bold text-[#16A34A] dark:text-emerald-300 block uppercase tracking-wider">
                    Estimated Net Profit (P&amp;L)
                  </span>
                  <div className="text-2xl font-black text-[#16A34A] dark:text-emerald-400 font-mono">
                    ${data.overview.estimatedNetProfit.current.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USDT
                  </div>
                  <span className="text-[10px] text-slate-500 dark:text-emerald-400/80 block">
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {data.inventoryTelemetry.hubs.map((hub) => (
              <div
                key={hub.hubCode}
                className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-3 shadow-xs relative group hover:border-slate-300 dark:hover:border-slate-700 transition-colors"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-500 dark:text-slate-400 font-mono">{hub.hubCode}</span>
                  <span className="bg-[#EEF4FF] dark:bg-blue-950/60 text-[#2F65F6] border border-blue-200 dark:border-blue-900/30 text-[10px] font-bold px-2.5 py-0.5 rounded-full font-mono">
                    {hub.totalUnits} Units
                  </span>
                </div>
                <div>
                  <h4 className="text-sm font-black text-slate-900 dark:text-white">{hub.hubName}</h4>
                  <div className="text-xl font-black text-slate-900 dark:text-slate-200 font-mono mt-1">
                ...
                <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-100 dark:border-slate-800/80">
                  ...
                  <span className={cn("font-bold font-mono", hub.lowStockItemsCount > 0 ? "text-amber-500" : "text-[#16A34A] dark:text-emerald-400")}>
          ...
          <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-5 shadow-xs">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <Package className="w-5 h-5 text-[#2F65F6]" />
                  <h3 className="text-base font-black text-slate-900 dark:text-white">Top Best-Selling Hardware SKUs</h3>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400">Ranked by units sold and gross USDT sales volume</p>
              </div>
              ...
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-200 text-xs pl-9 pr-3 py-2 rounded-xl outline-none focus:border-[#2F65F6] transition-colors"
                />
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 text-slate-500 dark:text-slate-400 font-bold uppercase text-[10px] tracking-wider">
                    ...
                    <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/60 transition-colors">
                      <td className="py-4 px-4 font-sans font-bold text-slate-900 dark:text-white">
                        <div className="flex items-center gap-3">
                          ...
                          <Link href={`/admin/products`} className="hover:text-[#2F65F6] transition-colors line-clamp-1 max-w-xs">
                            {p.title}
                          </Link>
                        </div>
                      </td>
                      <td className="py-4 px-4 text-slate-500 dark:text-slate-400">{p.sku}</td>
                      <td className="py-4 px-4 text-right text-slate-900 dark:text-white font-bold">${p.basePrice.toFixed(2)}</td>
                      {data.isSuperAdmin && (
                        <td className="py-4 px-4 text-right text-slate-500 dark:text-slate-400">
                          {p.factoryCost ? `$${p.factoryCost.toFixed(2)}` : "-"}
                        </td>
                      )}
                      <td className="py-4 px-4 text-right text-[#16A34A] dark:text-emerald-400 font-black">{p.unitsSold}</td>
                      <td className="py-4 px-4 text-right text-slate-900 dark:text-white font-bold">${p.grossRevenue.toFixed(2)}</td>
                      {data.isSuperAdmin && (
                        <td className="py-4 px-4 text-center">
                          <span className="bg-[#F0FDF4] dark:bg-emerald-950 text-[#16A34A] dark:text-emerald-300 border border-[#BBF7D0] dark:border-emerald-800 text-[10px] font-black px-2 py-0.5 rounded">
                            {p.marginPct}%
                          </span>
                        </td>
                      )}
                      <td className="py-4 px-4 text-center font-sans">
                        <span
                          className={cn(
                            ...
                            p.stock > 10 ? "bg-[#F0FDF4] dark:bg-emerald-500/10 text-[#16A34A] dark:text-emerald-400 border border-[#BBF7D0] dark:border-emerald-500/30" : "bg-[#FFF8EE] dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-[#FED7AA] dark:border-amber-500/30"
                          )}
                        >
                          {p.stock} units
                        </span>
                ...
                {data.categorySales.map((cat, idx) => (
                  <div key={idx} className="space-y-1.5 text-xs">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-slate-500 dark:text-slate-400">${cat.revenue.toFixed(2)} USDT ({cat.sharePct}%)</span>
                    </div>
                    <div className="w-full h-2 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden border border-slate-200 dark:border-slate-800">
                      <div className="h-full bg-[#2F65F6] rounded-full" style={{ width: `${cat.sharePct}%` }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
              ...
              <div className="space-y-3">
                {data.brandSales.map((brand, idx) => (
                  <div key={idx} className="space-y-1.5 text-xs">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-700 dark:text-slate-300 font-bold">{brand.name}</span>
                      <span className="font-mono text-slate-500 dark:text-slate-400">${brand.revenue.toFixed(2)} USDT ({brand.sharePct}%)</span>
                    </div>
                    <div className="w-full h-2 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden border border-slate-200 dark:border-slate-800">
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-5 shadow-xs">
              <div className="border-b border-slate-100 dark:border-slate-800 pb-4 space-y-1">
                <h3 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
                  <Truck className="w-5 h-5 text-[#2F65F6]" />
                  Order Pipeline Distribution
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">Fulfillment stages across Shenzhen &amp; Hong Kong hubs</p>
              </div>
              ...
              <div className="space-y-3">
                {data.orderStatusDistribution.map((st) => (
                  <div
                    key={st.status}
                    className="bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800/80 rounded-2xl p-3.5 space-y-2 hover:border-slate-300 dark:hover:border-slate-700 transition-colors"
                  >
                    ...
                    <div className="w-full h-2 bg-slate-200 dark:bg-slate-900 rounded-full overflow-hidden">
                      <div
                        className={cn(
                          ...
                          st.status === "paid" || st.status === "delivered" ? "bg-[#10B981]" : "bg-[#2F65F6]"
                        )}
            ...
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-5 shadow-xs">
              <div className="border-b border-slate-100 dark:border-slate-800 pb-4 space-y-1">
                <h3 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
                  <Globe className="w-5 h-5 text-[#16A34A] dark:text-emerald-400" />
                  Air Cargo Destinations (DDP)
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">Global buyer delivery distribution</p>
              </div>
              ...
              <div className="space-y-3">
                {data.geoDistribution.map((geo) => (
                  <div
                    key={geo.country}
                    className="flex items-center justify-between p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 text-xs"
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-8 h-8 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center font-mono font-bold text-slate-700 dark:text-slate-300">
                        {geo.code}
                      </span>
                      <span className="font-bold text-slate-900 dark:text-white">{geo.country}</span>
                    </div>
                    <div className="text-right font-mono">
                      <div className="font-bold text-[#16A34A] dark:text-emerald-400">${geo.revenue.toLocaleString()} USDT</div>
                      <div className="text-[10px] text-slate-400">{geo.orders} orders ({geo.sharePct}%)</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {activeTab === "binance" && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-2 shadow-xs">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Binance USDT Settled</span>
              <div className="text-2xl font-black text-[#16A34A] dark:text-emerald-400 font-mono">
              ...
              <span className="text-[11px] text-slate-400 block">Instant Web3 / App Checkout</span>
            </div>

            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-2 shadow-xs">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Reconciliation Variance</span>
              <div className="text-2xl font-black text-slate-900 dark:text-white font-mono">$0.00 USDT</div>
              <span className="text-[11px] text-[#16A34A] dark:text-emerald-400 block font-bold">100% Balanced Ledger</span>
            </div>

            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-2 shadow-xs">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Zero-Fee Savings</span>
              <div className="text-2xl font-black text-purple-600 dark:text-purple-400 font-mono">
                ${data.sourcingAndProfit.binancePayGasSavedUSDT.toFixed(2)} USDT
              </div>
              <span className="text-[11px] text-slate-400 block">Saved vs 2.9% Credit Card Processing</span>
            </div>
          </div>

          <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-5 shadow-xs">
            <h3 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
              <Coins className="w-5 h-5 text-[#16A34A] dark:text-emerald-400" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {data.binancePaymentsReconciliation.metrics.map((pm) => (
                <div
                  key={pm.status}
                  className="bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 space-y-2"
                >
                  <div className="flex items-center justify-between text-xs font-bold">
                    <span className={pm.color}>{pm.label}</span>
                    <span className="font-mono text-slate-500 dark:text-slate-400">{pm.pct}%</span>
                  </div>
                  <div className="text-xl font-black text-slate-900 dark:text-white font-mono">
                    ${pm.totalAmountUSDT.toLocaleString()} USDT
                  </div>
                  <span className="text-[10px] text-slate-400 font-mono block">{pm.count} transactions</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === "customers" && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-2 shadow-xs">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">New vs Returning Buyers</span>
              <div className="text-2xl font-black text-slate-900 dark:text-white font-mono">
                {data.customerRetention.returningBuyers}{" "}
                <span className="text-sm font-sans text-slate-500 dark:text-slate-400">/ {data.customerRetention.newBuyers} New</span>
              </div>
              <span className="text-[11px] text-[#16A34A] dark:text-emerald-400 font-bold block">
                {data.customerRetention.repeatPurchaseRatePct}% Repeat Purchase Rate
              </span>
            </div>

            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-2 shadow-xs">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Customer Lifetime Value (LTV)</span>
              <div className="text-2xl font-black text-[#16A34A] dark:text-emerald-400 font-mono">
                ${data.customerRetention.customerLtvUSDT.toFixed(2)} USDT
              </div>
              <span className="text-[11px] text-slate-400 block">Average cumulative spend per customer</span>
            </div>

            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-2 shadow-xs">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Recoverable Cart Revenue</span>
          ...
          <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-5 shadow-xs">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-4 space-y-1">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">High-intent customers who added items without completing payment</p>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-mono">
                  {data.abandonedCarts.recentAbandonedCarts.map((c) => (
                    <tr key={c.cartId} className="hover:bg-slate-50 dark:hover:bg-slate-950/60 transition-colors">
                      <td className="py-4 px-4 font-sans font-bold text-slate-900 dark:text-white">
                        <div>{c.userEmail}</div>
                        <span className="text-[10px] text-slate-400 font-normal">{c.customerName}</span>
                      </td>
                      <td className="py-4 px-4 text-slate-600 dark:text-slate-300 font-sans">
                        <span className="font-mono text-[#16A34A] dark:text-emerald-400 font-bold">{c.itemCount} items: </span>
                        {c.products.map((p) => p.title).join(", ")}
                      </td>
                      <td className="py-4 px-4 text-right text-[#16A34A] dark:text-emerald-400 font-black">
                        ${c.valueUSDT.toFixed(2)} USDT
                      </td>
                      <td className="py-4 px-4 text-right text-slate-500 dark:text-slate-400 text-[11px]">
                        {new Date(c.lastActive).toLocaleDateString()}
                      </td>
                      <td className="py-4 px-4 text-center">
                        <button
                          onClick={() => showToast(`Discount voucher email queued for ${c.userEmail}.`)}
                          className="bg-[#FFF8EE] dark:bg-amber-500/10 border border-[#FED7AA] dark:border-amber-500/30 text-amber-600 dark:text-amber-300 px-3 py-1 rounded-xl text-[11px] font-bold hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors cursor-pointer"
                        >
                          Send Incentive
                        </button>
                      </td>
      ...
      {activeTab === "refunds" && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-2 shadow-xs">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">30-Day Warranty Claims</span>
              <div className="text-2xl font-black text-slate-900 dark:text-white font-mono">
            ...
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-2 shadow-xs">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Refunded USDT</span>
              <div className="text-2xl font-black text-rose-600 dark:text-rose-400 font-mono">
                ${data.refundsAndRma.totalRefundedAmountUSDT.toFixed(2)} USDT
              </div>
              <span className="text-[11px] text-slate-400 block">Covered by Factory Quality Guarantee</span>
            </div>

            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-2 shadow-xs">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Inspection QC Speed</span>
              <div className="text-2xl font-black text-purple-600 dark:text-purple-400 font-mono">24.5 Hours</div>
              <span className="text-[11px] text-slate-400 block">Average Claim Resolution SLA</span>
            </div>
          </div>
            ...
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
              ...
              <div className="space-y-3">
                {data.refundsAndRma.evidenceBreakdown.map((ev, idx) => (
                  <div key={idx} className="space-y-1.5 text-xs">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-700 dark:text-slate-300 font-bold">{ev.type}</span>
                      <span className="font-mono text-slate-500 dark:text-slate-400">{ev.count} claims ({ev.pct}%)</span>
                    </div>
                    <div className="w-full h-2 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden border border-slate-200 dark:border-slate-800">
            ...
            <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs">
              ...
              <div className="space-y-3">
                {data.refundsAndRma.reasonsBreakdown.map((rsn, idx) => (
                  <div key={idx} className="space-y-1.5 text-xs">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-700 dark:text-slate-300 font-bold">{rsn.reason}</span>
                      <span className="font-mono text-slate-500 dark:text-slate-400">{rsn.count} orders ({rsn.pct}%)</span>
                    </div>
                    <div className="w-full h-2 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden border border-slate-200 dark:border-slate-800">
                      <div className="h-full bg-rose-500 rounded-full" style={{ width: `${rsn.pct}%` }} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      {activeTab === "schedules" && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 p-5 rounded-2xl shadow-xs">
            <div className="space-y-1">
              <h3 className="text-base font-black text-slate-900 dark:text-white">Automated Executive Email Schedules</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
            ...
            <button
              ...
              className="bg-[#2F65F6] text-white px-5 py-2.5 rounded-xl text-xs font-bold hover:bg-[#2563EB] shadow-blue-500/25 shadow-xs transition-colors flex items-center gap-2 cursor-pointer shrink-0"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {schedules.map((sched) => (
              <div
                key={sched.id}
                className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs relative group hover:border-slate-300 dark:hover:border-slate-700 transition-colors"
              >
                <div className="flex items-center justify-between">
                  <span className="bg-[#EEF4FF] dark:bg-blue-950/60 text-[#2F65F6] border border-blue-200 dark:border-blue-900/30 text-[10px] font-bold uppercase px-2.5 py-1 rounded-full font-mono">
                ...
                <div className="space-y-1">
                  <h4 className="text-sm font-black text-slate-900 dark:text-white">{sched.title}</h4>
                ...
                <div className="flex flex-wrap gap-1.5 pt-2">
                  {sched.metricsIncluded.map((m, idx) => (
                    <span
                      key={idx}
                      className="bg-slate-100 dark:bg-slate-950 text-slate-700 dark:text-slate-300 text-[10px] font-bold px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-800 font-mono"
                    >
                      {m}
                    </span>
                  ))}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800/80 text-xs">
                  ...
                  <button
                    ...
                    className="text-[#16A34A] dark:text-emerald-400 hover:text-emerald-500 font-bold flex items-center gap-1 text-xs cursor-pointer"
                  >
      ...
      <Modal
        ...
        size="lg"
      >
        <form onSubmit={handleSaveSchedule} className="space-y-4 pt-2 text-slate-900 dark:text-slate-100">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block">Report Schedule Title</label>
            <input
              required
              type="text"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block">Export Format</label>
              <select
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block">Recipient Emails (comma-separated)</label>
            <input
              required
              type="text"
      ...
      <Modal
        ...
        size="xl"
      >
        <div className="space-y-6 pt-2 text-slate-900 bg-white p-6 rounded-2xl">
          {/* Document Header */}
          <div className="flex items-center justify-between border-b pb-4">
            <div>
              <h2 className="text-2xl font-black text-[#2F65F6] tracking-tight">LENNOX CHINAMALL</h2>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
            <button
              ...
              className="px-4 py-2.5 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 transition-colors cursor-pointer"
            >
              Close
            </button>
            <button
              onClick={() => window.print()}
              className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-[#2F65F6] hover:bg-[#2563EB] shadow-blue-500/25 shadow-xs flex items-center gap-2 cursor-pointer transition-colors"
            >
      ...
      {toastMsg && (
        <div
          className={cn(
            "fixed bottom-6 right-6 z-50 px-5 py-3 rounded-2xl text-xs font-bold shadow-xl flex items-center gap-3 animate-in fade-in slide-in-from-bottom-3 border",
            toastMsg.type === "error" ? "bg-rose-600 text-white border-rose-500" : "bg-[#16A34A] text-white border-emerald-500"
          )}
        >
