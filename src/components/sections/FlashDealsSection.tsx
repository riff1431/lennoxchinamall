  ...
  return (
    <section className="relative rounded-3xl overflow-hidden shadow-2xl">
      ...
      <div className="relative z-10">
        {/* ── Header Bar ── */}
        <div className="px-5 sm:px-7 pt-6 sm:pt-7 pb-4 sm:pb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-5">
            {/* Glowing icon + title */}
            <div className="flex items-center gap-2.5">
              <div className="animate-flash-glow">
                <Zap className="w-7 h-7 sm:w-8 sm:h-8 fill-amber-400 text-amber-400" />
              </div>
          ...
          {/* View All CTA with glow border */}
          <Link
            href="/categories/flash-deals"
            className="flash-cta-glow text-xs sm:text-sm font-black text-white flex items-center gap-1.5 bg-white/10 hover:bg-white/15 px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl transition-colors border border-white/15 backdrop-blur-sm"
          >
        ...
        {/* ── Product Grid ── */}
        <div className="px-5 sm:px-7 pb-6 sm:pb-7 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {deals.map((product, idx) => (
            <div
              key={product.id}
              className="flash-glass-card rounded-2xl overflow-hidden animate-flash-card-enter"
