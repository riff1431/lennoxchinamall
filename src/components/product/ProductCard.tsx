...
export function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  return (
    <div
      className="group relative bg-white rounded-2xl border border-slate-200/90 overflow-hidden card-hover-effect flex flex-col justify-between shadow-xs hover:border-[#00143D]/30 hover:shadow-xl transition-all duration-300 font-montserrat"
    ...
    >
      <Link href={`/products/${product.slug}`} className="block relative">
        ...
        <div className="relative w-full aspect-square bg-[#F8FAFC] overflow-hidden product-image-container">
          ...
          <div className="absolute top-2.5 left-2.5 flex flex-col gap-1.5 z-10">
            {discount > 0 && (
              <span className="bg-[#FF1028] text-white text-[10px] sm:text-[11px] font-black px-2 py-0.5 rounded-md shadow-md uppercase tracking-wider">
                -{discount}% OFF
              </span>
            )}
            {product.is_flash_deal && (
              <span className="bg-[#00143D] text-amber-300 text-[9px] font-extrabold px-2 py-0.5 rounded shadow-xs flex items-center gap-1 border border-amber-300/30">
          ...
          {/* Video Preview Tag */}
          {videoCount > 0 && (
            <div className="absolute bottom-2.5 left-2.5 bg-[#00143D]/80 backdrop-blur-xs text-white text-[10px] font-bold px-2 py-0.5 rounded-md flex items-center gap-1.5 z-10 border border-white/15">
          ...
          <button
            ...
            className="absolute top-2.5 right-2.5 w-8 h-8 rounded-full bg-white/95 backdrop-blur-md flex items-center justify-center text-slate-400 hover:text-[#FF1028] hover:scale-115 transition-all z-10 shadow-md cursor-pointer border border-slate-100"
        ...
        <div className="p-3.5 sm:p-4 flex flex-col gap-1.5 flex-1">
          {/* Title */}
          <h3 className="text-xs sm:text-sm font-bold text-slate-900 line-clamp-2 leading-snug group-hover:text-[#FF1028] transition-colors">
            {product.title}
          </h3>
          ...
          <div className="flex items-center justify-between text-xs pt-0.5">
            ...
            {product.sold_count > 0 && (
              <span className="text-[11px] text-slate-500 font-bold bg-slate-100 px-1.5 py-0.2 rounded">
          ...
          <div className="mt-1.5 pt-1.5 border-t border-slate-100">
            ...
            {savingsAmount > 0 && (
              <div className="text-[10px] font-extrabold text-[#FF1028] mt-0.5">
          ...
          <div className="mt-2 space-y-1 pt-1.5 border-t border-slate-100 text-[10px] text-slate-500 font-semibold">
            ...
            <div className="flex items-center gap-1.5 text-[#10B981] font-bold">
              ...
              <span>Direct China Factory Gate Inspected</span>
            </div>
          </div>
        </div>
      </Link>
      ...
      <div className="px-3.5 pb-3.5 sm:px-4 sm:pb-4 pt-1">
        <button
          ...
          className={`w-full py-2.5 px-3 rounded-xl text-xs font-black flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm active:scale-95 ${
            justAdded
...
export function ProductCardSkeleton() {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden flex flex-col justify-between shadow-xs animate-pulse font-montserrat">
